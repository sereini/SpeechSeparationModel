...
model to separate the audio signal of one speaker from background noise 
(in particular from other conversations).
...
'''
------------------------------------------------------------------------------
desc:            defines the loss as mse between label and prediction
param: 
    calc_mask1:  prediction of the avspeech model
    aud_mix:     superimposed audio signal (desired speaker and one other talker)
    lbl_aud1:    label of desired speaker

return:          loss (scalar)
------------------------------------------------------------------------------
'''
def loss_mean_squared_error(calc_mask1, aud_mix, lbl_aud1):
...
def cnn_model_fn(features, labels, mode):
    ...
    # fully connected layer for complex mask
    with tf.variable_scope('output'):
        ...
        complex_mask_out = tf.reshape(complex_mask, [-1,fc3.shape[1],257,2])
        complex_mask_out_t = tf.transpose(complex_mask_out,[0,2,1,3], name="output_masks")
        
    # define a name for output tensor (contains the complex mask)
    ...
    with tf.name_scope('loss'):
        loss = loss_mean_squared_error(complex_mask_out_t, aud_mix, aud1)
...
def traineval(model_dir):
    ...
    train_dir_tfrecord = ["D:/BA_LipRead/03_DeepNet/data_tfrecord_train_298/",
                          "D:/BA_LipRead/03_DeepNet/data_tfrecord_train_1/",
                          "C:/BA_LipRead/data_tfrecord_train_3/",
                          "C:/BA_LipRead/data_tfrecord_train_4/",
                          "C:/BA_LipRead/data_tfrecord_train_5/"]       

    # define the train data dirs as a list (absolute path)         
    eval_dir_tfrecord = ["D:/BA_LipRead/03_DeepNet/data_tfrecord_eval_298/",
                         "C:/BA_LipRead/data_tfrecord_eval_3/",
                         "C:/BA_LipRead/data_tfrecord_eval_4/"] 
...
def main(unused_argv):
    ...
    # define model dir
    model_dir="./logs/model_AVSpeech_oneSpeaker/"
