...
@author: sscherrer
filename: predict_AVSpeech_twoSpeakers

This program is used to predict the separated audio tracks from a given TFRecord (evaluate data).
The pretrained model is trained to separate two independet speakers.
    ...
    for test_rec in test_record:
        ...
        with tf.Graph().as_default():
            with tf.Session() as sess:
                ...
                vid2 = tf.get_default_graph().get_tensor_by_name("vid2:0")
                ...
                feed_dict = {vid1:  np.reshape(pred_video[0,:,:],[-1,pred_video.shape[1],pred_video.shape[2]]),
                             vid2:  np.reshape(pred_video[1,:,:],[-1,pred_video.shape[1],pred_video.shape[2]]),
                             aud_mix: np.reshape(pred_audio,[-1,pred_audio.shape[0],pred_audio.shape[1],pred_audio.shape[2]])}
                
                # use the avspeech model to calcualte the separated audiosignals
                masks = sess.run(prediction_masks, feed_dict=feed_dict)
                    
                # calculate complex mask from splitted output (real and imaginary parts are stacked)
                masks = np.reshape(masks,[masks.shape[1],masks.shape[2],masks.shape[3],masks.shape[4]])
                mask1 = np.vectorize(complex)(masks[:,:,0,0],masks[:,:,1,0])
                mask2 = np.vectorize(complex)(masks[:,:,0,1],masks[:,:,1,1])
                ...
                print("diff2: ",np.mean(np.square(np.abs(pred2-aud2))))
                print("total: ", np.mean(np.add(np.square(np.abs(pred1-aud1)),np.square(np.abs(pred2-aud2)))))
...
model_dir = "./model/model_AVSpeech_001/"
